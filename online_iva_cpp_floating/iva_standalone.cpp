// =============================================================================
// Online AuxIVA-IP / AuxIVA-ISS  -- single-file standalone, fp32
// =============================================================================
//
...
//
//   Build:  g++ -std=c++17 -O3 iva_standalone.cpp -o iva_standalone
//           cl /std:c++17 /O2 /EHsc iva_standalone.cpp        (MSVC)
//
//   Run:    iva_standalone <input.wav> <output_prefix> <ip|iss>
//                          [n_fft=2048] [hop=n_fft/2] [n_iter=1]
//                          [alpha=0.995] [model=laplace]
//
//   e.g.    iva_standalone mixture.wav out iss
...
// See reset(): opt-in, signal-scaled V initialization. Off by default so this
// file matches ../online_iva_cpp/iva_standalone_double.cpp out of the box and
// the two can be compared precision-only.
...
// PRECISION POLICY
//
//   real        (float)  - the signal path: every per-frame computation
//   setup_real (double)  - one-off table construction only: FFT twiddles,
//                          window tables, WAV sample conversion
//
// The split is deliberate. The signal path is fp32 because that is what an
// ARM+NEON target wants (4 lanes vs 2, and fp64 is often half-rate or worse on
// embedded cores). The setup computations run ONCE in a constructor, never in
// the per-frame path, so using double there costs nothing at runtime and keeps
// the tables accurate -- computing a twiddle angle in fp32 rounds 2*pi before
// multiplying, which is measurably worse for no benefit.
//
// Define PIVA_NO_FP64 if the target has no fp64 at all (it will otherwise
// soft-float those sites). The timing instrumentation in main() also uses
// double; it is measurement only and can simply be deleted.
#ifdef PIVA_NO_FP64
using setup_real = float;
#else
...
//
// Used for sums that run over n_freq bins (1025 at n_fft = 2048). Measured on
// this project's 60 s test signal, naive fp32 summation of `r2` gives
// BIT-IDENTICAL output -- the error is about sqrt(n)*eps ~ 4e-6 relative,
// which does not move the weight. So this is cheap insurance against harsher
// input (very long frames, wider dynamic range), not a fix for an observed
// defect. Three extra flops per term, and no wider type needed.
//
...
inline WavData read_wav(const std::string& filename) {
  ...
  for (size_t i = 0; i < wav.samples.size(); i++) {
    ...
    if (is_float && bits == 32) {
      ...
      // target must be exactly 8 bytes -- it CANNOT be `setup_real`, which is
      // 4 bytes under -DPIVA_NO_FP64 (and some non-conforming embedded
      // toolchains, e.g. AVR-GCC and several TI DSP compilers, make `double`
      // a 32-bit alias for float too). Either way, copying 8 bytes into a
      // 4-byte object is a stack overwrite, not a precision loss.
...
//
// `eps` is passed in already scaled to the signal (see eps_abs()). It must NOT
// be derived from r itself: phi(r) = 1/(2r) is the Laplace auxiliary weight,
// and self-normalizing r would change the contrast function's meaning and
// break the property that makes the AuxIVA majorization valid. Scaling only
// the floor keeps phi's shape intact.
...
class OnlineAuxIVABase {
 public:
  ...
  OnlineAuxIVABase(size_t n_freq, size_t n_chan, size_t n_iter = 1,
                   ...
                   real eps_rel = 1e-6f, real silence_rel = 1e-6f, real degen_rel = 1e-6f,
                   real v_init_rel = 1.0f)
      : n_freq_(n_freq),
        n_chan_(n_chan),
        n_src_(n_chan),
        n_iter_(n_iter),
  ...
  // matching the reference folder build and the double standalone.
  //
  // -DPIVA_SCALED_V_INIT instead leaves V at zero here and lets
  // scale_v_to_signal() set its scale from the first active frames. That is
  // NOT the default because it is a trade, not a clear win: measured on this
  // project's 60 s material it takes steady interference 0.052 -> 0.016, but
  // target fidelity 0.835 -> 0.731 and roughly doubles convergence time.
  // See scale_v_to_signal() for what it does and does not fix.
  void reset() {
  ...
  // det(a*d - b*c) for complex values, entirely in fp32.
  //
  ...
  void project_back_frame(const cplx* y, cplx* y_scaled, size_t ref = 0) {
    ...
    for (size_t f = 0; f < n_freq_; f++) {
      ...
      for (size_t c = 0; c < n; c++) e[c] = (c == ref) ? 1.0f : 0.0f;
 ...
 protected:
  ...
  // Both the per-frame sum and the running peak are fp32; the sum is
  // compensated because it runs over n_freq*n_chan terms.
  bool frame_is_active(const cplx* x_cur) {
    ...
    peak_power_ = std::max(peak_power_ * 0.999f, power);  // forgets far slower than alpha
    if (silence_rel_ <= 0.0f) return true;
    return power > peak_power_ * silence_rel_;
  }

  // Absolute floor for phi(), derived from the gain-tracking peak so it scales
  // with the input. r = sqrt(sum_f |y_f|^2) has magnitude ~ sqrt(n_freq *
  // power), so that is the reference; eps_rel_ is the dimensionless fraction
  // of it below which r is treated as zero.
  real eps_abs() const {
    return eps_rel_ * std::sqrt(peak_power_ * real(n_freq_));
  }

  // Scale V's initialization to the signal.
  //
  // THIS IS NOT COSMETIC. V starts isotropic (c*I). If c is far larger than
  // the per-frame contribution weight*xxH, then V_m stays ~ c*alpha^t*I for
  // EVERY source -- isotropic and identical across m. ISS then computes, for
  // m != k,
  //     v_m = (w_m^H V_m w_k)/(w_k^H V_m w_k) = (w_m^H w_k)/||w_k||^2
  // because the scalar c cancels exactly. And v_k -> 0 once the normalization
  // settles ||w_k||^2 = 1/c. So the steering still runs and still writes to W,
  // but carries ZERO information about the input: it converges to a fixed
  // point determined only by the initial W. Measured with a fixed absolute
  // init of 0.01, a -120 dBFS input produced output frozen to three digits
  // across all 12 repetitions -- a null update that looks like a working
  // algorithm.
  //
  // WHAT THIS DOES NOT FIX -- measured, do not assume otherwise. An earlier
  // version of this comment claimed the transient becomes O(1/(1-alpha))
  // frames "at any gain". That is false. c0 below is proportional to the input
  // gain (verified: exactly 1e-6x going from 0 to -120 dBFS), but the
  // recursion's equilibrium is proportional to gain SQUARED. ISS pins
  // w_k^H V_k w_k = 1, an absolute target, so W ~ 1/sqrt(V); then r = ||Wx||
  // and weight = (1-alpha)/(2r), which makes the data term weight*xxH scale as
  // gain^(a/2+1) when V ~ gain^a. The fixed point is a = 2, not a = 1.
  //
  ...
  //
  // Rescaling V is safe at any time: scaling V_k only rescales row k of W, and
  // project_back_frame() sets the output level regardless. That is why this
  // can keep revising upward during the early frames -- if the first active
  // frame is room tone rather than speech, a later louder frame corrects it.
  void scale_v_to_signal() {
    ...
    if (!(mean_power > 0.0f)) return;

    // steady state of the recursion is ~ phi(r)*xxH, and r ~ sqrt(total
    // power), so this is the natural magnitude for V's diagonal
    const real r_ref = std::sqrt(mean_power * real(n_freq_ * n));
    const real c0 = v_init_rel_ * mean_power / (2.0f * std::max(r_ref, eps_abs()));
    if (!(c0 > v_ref_)) return;  // only ever revise upward

    if (v_ref_ > 0.0f) {
      const real g = c0 / v_ref_;  // already initialized: rescale in place
      for (cplx& v : V_) v *= g;
    } else {
      std::fill(V_.begin(), V_.end(), cplx(0.0f));
      for (size_t k = 0; k < n_src_; k++)
  ...
  // r2 runs over n_freq bins, so it is accumulated with Kahan compensation --
  // naive fp32 summation over 1025 terms loses enough precision to visibly
  // perturb the weight, and this file uses no wider type to fall back on.
  void update_covariance(const cplx* x_cur, const cplx* x_next, size_t k) {
    ...
    for (size_t f = 0; f < n_freq_; f++) {
      ...
      r2.add(0.5f * (std::norm(y_cur) + std::norm(y_next)));
    }
    const real weight = (1.0f - alpha_) * phi(std::sqrt(r2.get()), model_, n_freq_, eps_abs());
  ...
  real alpha_;
  Model model_;
  real eps_rel_, silence_rel_, degen_rel_, v_init_rel_, v_ref_;
  int v_scale_frames_seen_;
  // how many early active frames may revise V's initial scale upward
...
class AuxIVAISSOnline : public OnlineAuxIVABase {
 public:
  AuxIVAISSOnline(size_t n_freq, size_t n_chan, size_t n_iter = 1,
                  real alpha = 0.995f, Model model = Model::Laplace,
                  real eps_rel = 1e-6f, real silence_rel = 1e-6f,
                  real degen_rel = 1e-6f, real v_init_rel = 1.0f)
      : OnlineAuxIVABase(n_freq, n_chan, n_iter, alpha, model, eps_rel, silence_rel,
                         degen_rel, v_init_rel),
        wk_(n_chan),
        v_(n_chan) {}
  ...
  void update_demixing(size_t k) override {
    ...
    for (size_t f = 0; f < n_freq_; f++) {
      ...
      for (size_t m = 0; m < n_src_; m++) {
        ...
        real denom = 0.0f, trace = 0.0f;
        cplx numer = 0.0f;
        for (size_t r = 0; r < n; r++) {
          cplx Vw = 0.0f;  // (V_m w_k)_r
        ...
        if (!(trace > 0.0f) || !(denom > degen_rel_ * trace)) {
          degenerate = true;
          break;
        }
        v_[m] = (m == k) ? cplx(1.0f - 1.0f / std::sqrt(denom)) : numer / denom;
      }
      if (degenerate) {
        ++bins_skipped_;
        continue;
 ...
 protected:
  void update_demixing(size_t k) override {
    ...
    for (size_t f = 0; f < n_freq_; f++) {
      ...
      for (size_t c = 0; c < n; c++) w_[c] = (c == k) ? 1.0f : 0.0f;
      ...
      const real scale = 1.0f / std::sqrt(denom);
 ...
 public:
  StftAnalysis(size_t n_fft, size_t hop, size_t n_chan)
      : n_fft_(n_fft),
        ...
        scratch_(n_fft),
        cached_frame_(n_chan * (n_fft / 2 + 1), cplx(0.0f)),
        primed_(false) {
  ...
  void push(const real* samples, cplx* x_cur, cplx* x_next) {
    for (size_t c = 0; c < n_chan_; c++) {
      ...
      for (size_t i = 0; i < n_fft_; i++) scratch_[i] = cplx(buf[i] * window_[i], 0.0f);
...
class StftSynthesis {
 public:
  ...
  void push(const cplx* frame, real* samples) {
    const real inv_n = 1.0f / real(n_fft_);  // the inverse FFT is unnormalized
    for (size_t c = 0; c < n_chan_; c++) {
...
int main(int argc, char** argv) {
  ...
  const real alpha = argc > 7 ? real(std::stod(argv[7])) : 0.995f;
  ...
  try {
    ...
    std::cout << "Input: " << n_chan << " channels, " << n_samples << " samples at "
              << wav.samplerate << " Hz  [standalone fp32]\n";
    if (n_chan < 2) {
