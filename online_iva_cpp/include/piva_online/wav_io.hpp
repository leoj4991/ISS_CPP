...
inline WavData read_wav(const std::string& filename) {
  ...
  for (size_t i = 0; i < wav.samples.size(); i++) {
    ...
    if (is_float && bits == 32) {
      ...
      // A 64-bit float WAV stores IEEE-754 binary64, so the memcpy target must
      // be exactly 8 bytes. Some non-conforming embedded toolchains (AVR-GCC,
      // several TI DSP compilers) make `double` a 32-bit alias for float, and
      // copying 8 bytes into it is a stack overwrite, not a precision loss.
      static_assert(sizeof(double) == 8,
                    "64-bit float WAV input requires a 64-bit double");
      std::memcpy(&v, p, 8);
    } else if (bits == 8) {
