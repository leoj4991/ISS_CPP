...
//
// DOUBLE-PRECISION build -- the reference implementation. Validate optimized
// or reduced-precision ports against this one.
//
...
// This is the double-precision reference build. The single-precision port
// lives in ../../online_iva_cpp_floating (and a dependency-free single-file
// version of it in that folder's iva_standalone.cpp).
using real = double;
...
// which is enormous -- that is why the activity gate exists, rather than
...
class OnlineAuxIVABase {
 public:
  // eps: absolute floor inside phi(). Note it is NOT gain-relative, so a very
  // quiet recording can in principle reach it; measured on this project's
  // material it never fires between 0 and -120 dBFS.
  //
  ...
  void project_back_frame(const cplx* y, cplx* y_scaled, size_t ref = 0) {
    ...
    if (n == 2) {
      ...
      for (size_t f = 0; f < n_freq_; f++) {
        ...
        // RELATIVE degeneracy test. An exact-zero test is close to useless
        // here: a near-singular W does not land on exactly 0.0, it lands on a
        // tiny non-zero value, sails past `det == 0`, and r0/det then produces
        // an enormous scale factor on that bin -- one such spike is enough to
        // wreck a whole file once the output is peak-normalized. Higher
        // precision makes exact zero LESS likely, not more, so double does not
        // help. Compare |det|^2 against the size of the products that formed
        // it; squared magnitudes throughout, so there is no sqrt in the loop.
        const double scale2 = std::norm(std::complex<double>(a)) *
                                  std::norm(std::complex<double>(d)) +
                              std::norm(std::complex<double>(b)) *
                                  std::norm(std::complex<double>(c));
        if (!(std::norm(det) > double(degen_rel_) * double(degen_rel_) * scale2)) {
          ++pb_degenerate_;
 ...
 protected:
  ...
  bool frame_is_active(const cplx* x_cur) {
    ...
    if (silence_rel_ <= 0.0) return true;  // gate disabled
  ...
  void update_covariance(const cplx* x_cur, const cplx* x_next, size_t k) {
    ...
    real weight = (1.0 - alpha_) * phi(real(std::sqrt(r2)), model_, n_freq_, eps_);
